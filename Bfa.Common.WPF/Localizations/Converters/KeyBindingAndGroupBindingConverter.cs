// -----------------------------------------------------------------------
// <copyright file="KeyBindingAndGroupBindingConverter.cs" company="bfa solutions ltd">
...
namespace Bfa.Common.WPF.Localizations.Converters
{
    ...
    /// <summary>
    ///     The KeyBinding and GroupBinding converter class
    /// </summary>
    /// <seealso cref="KeyBindingConverterBase{KeyBindingAndGroupBindingConverter}" />
    internal class KeyBindingAndGroupBindingConverter : KeyBindingConverterBase<KeyBindingAndGroupBindingConverter>
    {
        /// <summary>
        ...
        protected override string Convert(object[] values, LocTextBindingExtension parameter, CultureInfo culture)
        {
            ...
            if (!TryGetKey(values[1], out var group))
            {
                return parameter.UseDefaultIsEmpty ? parameter.Default : null;
            }
            ...
            try
            {
                return string.Format(formatter, GetFormatterObjects(values, parameter, 2));
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
